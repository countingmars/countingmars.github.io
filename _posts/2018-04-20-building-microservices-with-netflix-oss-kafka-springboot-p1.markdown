...
나는 지난 몇 달간의 리서치와 프로젝트 경험을 통해 얻은 지식을 여기에 정리해 보겠다. 
...
목표는 신규 사용자가 등록된 후에 확인 메일을 보내는 등록 시스템을 만드는 것이다. 아래의 컴포넌트들은 이 시스템의 구성원들이다:
1. Service registry (Eureka) - 모든 서비스들은 스스로를 이곳에 등록할 것이다. 
2. Config server (Spring Cloud Config) - 모든 서비스들은 이곳에서 자신들의 설정을 가져올 것이다. Config server 는 설정 파일을 git repository 에 저장할 것이다.
...
일감에는, 이러한 주소 정보를 설정 파일에 저장(keep)함으로써 해결할 수 있을 것 같을테고, 아마 2 ~ 4 개의 서비스라면 그럴 것이다.


하지만 30개 이상, 심지어 100개 이상의 서비스라면 configuration 헬을 가져올테고, Cloud-based 시스템이라면 사태는 더 심각해질거다.  
...
이 어노테이션은 Eureka Server 관련된 설정을 활성화(activate)할 것이다.
...
##### 빌드, 실행 및 확인
1. `mvn clean install`
2. target 폴더로 이동해서 `java –jar ms–discovery–0.0.1–SNAPSHOT.jar`
...
Spring Cloud config server 는 피보탈(Pivotal)이 제공하는 Externalized Configuration Pattern 이다. 
Spring Cloud config server 는 서비스를 위한 각각의 환경 설정 파일을 git repository 에 저장하고, 서비스가 시작될 때  
서비스가 설정 서버에게 적절한 설정 정보를 요청한다.
...
ms-config-server 를 제외한 나머지 서비스들은 설정파일을 위한 git repository 가 필요하며, ms-config-properties 라고 부르자. 
...
Application.java 에 @EnableEurekaClient 를 추가해서 Eureka 의 클라이언트로 설정하자.
디폴트로, Eureka 서버를 http://localhost:8761 에서 찾을 것이다.  
그리고 @EnableConfigServer 를 추가해서 config server 를 활성화시켜야 한다. 
...
디폴트로, config server 는 8888 포트로 구동된다.
